...
Fonts use `font-display: optional`. Preloading makes the custom faces available early on fast loads. If a font arrives late, that page view keeps its fallback font instead of shifting text and screenshots. The shipping hero image also has an explicit CSS aspect ratio.
